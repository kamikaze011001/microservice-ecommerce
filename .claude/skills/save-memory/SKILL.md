...
name: save-memory
description: >
  Use when closing/ending a session, or when the Stop hook nudges "save-memory" —
  distill the session's work into .claude/memory/ so the next session can catch up.
  Trigger phrases: "save memory", "end session", "save session".
template_version: 2.4.0
---

# Save Memory — microservice-ecommerce
...
- A rule that must reach **subagents** belongs in `CLAUDE.md`, not here — memory only holds the
  rationale (see `.claude/memory/README.md`).
...
New conventions or gotchas encountered (e.g. a build flag, a syntax that causes a bug). Same
