...
- [an-oracle-can-validate-a-command-nobody-runs](conventions/an-oracle-can-validate-a-command-nobody-runs.md) — aws-diff-test rendered a helm invocation the live apply never issued; flags for both modes belong in one shared array
- [decomposing-a-wrapper-drops-what-it-did-besides-calling-steps](conventions/decomposing-a-wrapper-drops-what-it-did-besides-calling-steps.md) — running up-all.sh's steps individually lost its PUSH default and its Step 0 env preflight, twice
- [a-golden-fixture-proves-the-renderer-not-the-inputs](conventions/a-golden-fixture-proves-the-renderer-not-the-inputs.md) — the audit certified mail CONFIGURED from a golden built out of test fixtures; the real run failed on an unset env var
