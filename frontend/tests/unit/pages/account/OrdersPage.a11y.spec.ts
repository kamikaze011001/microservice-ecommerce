...
// <main> landmark. Mounted in isolation the top-level masthead/content isn't wrapped
// by that main, so the layout-owned landmark rules are scoped out here. The page owns
// its own <h1> (THE LEDGER), so page-has-heading-one is NOT scoped out — the guard
// asserts it below. (This iteration ALSO fixed a real bug: OrdersPage previously
// rendered its own <main>, nesting a second main landmark inside AccountLayout's — now a <div>.)
