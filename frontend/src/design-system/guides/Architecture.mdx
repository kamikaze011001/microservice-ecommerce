...
Decision rationale lives in `docs/adr/` (repo path — see **Decision records** at the
end of this guide).
...
Architecture decisions behind the picks in the stack table above are recorded as ADRs
in `docs/adr/` (repo path) — that's the decision-record home; ADRs stay in the repo
rather than moving into Storybook.
