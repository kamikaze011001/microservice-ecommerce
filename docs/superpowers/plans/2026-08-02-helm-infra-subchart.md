...
| `deploy/charts/microecom/charts/infra/templates/_helpers.tpl` | `microecom.fqdn` (cross-service addressing) + `microecom.waitForKafka` / `microecom.ensureCompacted` (shared initContainers) |
...
- Modify: `deploy/charts/microecom/charts/infra/templates/_helpers.tpl` (append `microecom.waitForKafka`)
- Test: `deploy/charts/microecom/tests/render-test.sh` (append a section)
...
- [ ] **Step 4: Add the `wait-for-kafka` helper to `_helpers.tpl`**

Three workloads need this identical initContainer — `kafka-exporter` here, plus `schema-registry` and `kafka-connect` in Task 5. Define it once. Append to `deploy/charts/microecom/charts/infra/templates/_helpers.tpl`:
...
`nindent 8` is correct for a pod spec's `initContainers:` list: the key sits at 6 spaces, its items at 8.
...
- Modify: `deploy/charts/microecom/charts/infra/templates/_helpers.tpl` (append `microecom.ensureCompacted`)
- Test: `deploy/charts/microecom/tests/render-test.sh` (append a section)
...
- [ ] **Step 3: Add the `ensure-compacted` helper to `_helpers.tpl`**

Both workloads run the identical repair loop over a different topic list, so the list is the only parameter. Append to `deploy/charts/microecom/charts/infra/templates/_helpers.tpl`:
...
```yaml
      ...
      initContainers:
        {{- include "microecom.waitForKafka" (dict "kafka" $kafka) | nindent 8 }}
        {{- include "microecom.ensureCompacted" (dict "kafka" $kafka "topics" "_schemas") | nindent 8 }}
      containers:
```
...
- [ ] **Step 5: Write `kafka-connect.yaml` with three waits**

The same two helpers with Connect's own topic list, then a third wait, then the existing `install-plugins`. initContainers run sequentially, so these prepend:

```yaml
      enableServiceLinks: false
      initContainers:
        {{- include "microecom.waitForKafka" (dict "kafka" $kafka) | nindent 8 }}
        {{- include "microecom.ensureCompacted" (dict "kafka" $kafka "topics" "connect-configs connect-offsets connect-status") | nindent 8 }}
...
followed by the existing `install-plugins` initContainer copied verbatim. Add `{{- $sr := include "microecom.fqdn" (dict "name" "schema-registry" "namespace" $ns) -}}` at the top. `wait-for-schema-registry` stays inline rather than becoming a helper — it has exactly one call site, and it deliberately reuses `confluentinc/cp-kafka-connect:7.6.1`, the main container's own image, so no extra pull. Set `CONNECT_BOOTSTRAP_SERVERS: "{{ $kafka }}"`.

- [ ] **Step 6: Run the harness — expect pass**

- [ ] **Step 7: Confirm the initContainer order on kafka-connect**
