...
#
# ("deploy", "k8s") / ("bootstrap", "k8s") repointed in Phase 8 Task 1 (the
# k8s Helm cut-over): VERB_deploy_k8s / VERB_bootstrap_k8s now resolve to
# k8s-apps-helm / k8s-bootstrap-helm instead of k8s-apps / k8s-bootstrap, so
# the right-hand side is baseline/k8s-apps-helm.txt / baseline/
# k8s-bootstrap-helm.txt — fresh baselines captured at cut-over time (see
# capture-baseline.sh), not the frozen pre-cut-over evidence. The old
# k8s-apps.txt / k8s-bootstrap.txt baselines are untouched and still exist
# (those targets aren't deleted), they're just no longer what these two pairs
# diff against.
#
# ("bootstrap", "compose") repointed in Phase 8 Task 2 (the compose
# cut-over): bootstrap-compose's own recipe was repointed off vault-import /
# seed-data onto secrets-seed / seed (see the Makefile target's comment), so
# baseline/bootstrap.txt — frozen PRE-CONVERSION evidence for the Task 3
# (Phase 6) verbatim move into bootstrap-compose, and explicitly never
# regenerated by capture-baseline.sh, even under FORCE=1 — no longer
# describes what bootstrap-compose actually does, by design. Unlike the k8s
# pair above there is no separate old target left unchanged to keep
# bootstrap.txt honest, so this pair now diffs against baseline/
# bootstrap-compose.txt instead — the SAME baseline capture-baseline.sh's own
# comment already designates as the ongoing, re-capturable oracle for this
# target ("Ongoing regeneration targets ... bootstrap-compose"), regenerated
# at this cut-over. bootstrap.txt itself is left untouched (still historical
# proof of the Task 3 move, just no longer wired into this table).
# ---------------------------------------------------------------------
PAIRS = [
    ("deploy",       "compose", "svc-start.txt"),
    ("deploy",       "k8s",     "k8s-apps-helm.txt"),
    ...
    ("bootstrap",    "compose", "bootstrap-compose.txt"),
    ("bootstrap",    "k8s",     "k8s-bootstrap-helm.txt"),
...
for verb, env, baseline_name in PAIRS:
    ...
    # Bare `make -n <target>` here, NOT `ENV=<env>`. Phase 8 Task 1 changed
    # the dispatch macro's recursive $(MAKE) call to explicit `ENV=` (empty),
    # not an inherited passthrough: k8s-apps-helm / k8s-bootstrap-helm read
    # bare $(ENV) themselves (default "local-k8s"), a DIFFERENT vocabulary
    # from the verb-env this loop iterates over (compose/k8s/aws) — GNU
    # Make's automatic command-line-var forwarding to sub-makes used to leak
    # the literal string "k8s" into that read, sending it looking for a
    # nonexistent envs/k8s.yaml. The Makefile fix strips it; this proof's
    # left-hand side must match what dispatch actually runs post-fix, which
    # is always the target's own default, never the verb-env string. The
    # other 12 resolved targets are ENV-blind (never read $(ENV) at all) so
    # dropping the explicit ENV= here changes nothing for them.
    live = mk([target])
