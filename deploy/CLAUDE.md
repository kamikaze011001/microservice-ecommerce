# deploy/ — guidance for Claude

Guidance for the consolidated deploy tree. See `README.md` for the usage/command
reference and `../CLAUDE.md` for project-wide conventions.

> **Migrated from `k8s/CLAUDE.md` (Phase 8 of the deploy refactor, 2026-08-14.)**
> All 19 SCARs below are preserved verbatim. Several reference paths that Phase 8
> deleted — `k8s/`, `docker/vault-configs/`, `docker/*.sql|json`, `scripts/seed/`.
> **The scars were kept anyway on purpose: a scar documents a trap in the problem
> domain, not a fact about a directory.** The next person will reach for the same
> pattern in the new tree and hit the same wall — that kustomize refuses out-of-tree
> file references, that Confluent cp-* images need `enableServiceLinks: false`, that a
> load-slave/mutate/save-master transaction self-deadlocks under Atomikos XA. Where a
> referenced path no longer exists, treat the path as historical and the lesson as
> current.
